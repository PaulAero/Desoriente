...
class HorizonArtificiel:
    ...
    def mode_pression(self):
        ...
        if self.text_pression == "QNH":
            ...
            import tkinter as tk
            from keyboard import CreationPad
            import time
            self.text_pression = "QNH"
            pression_affichage.config(text = self.text_pression)
            pad=CreationPad(self.root2)
            self.valeur_pression_entree= pad.recuperer_valeur()
...
# #             canvas.create_text(arrivee_x,arrivee_y*1.015,text=str(i-9),fill='#DDD',font='bold')
 ...
 #This method gets the gps position
    ...
    def horizon_artificiel(self):
        ...
        self.bouton_carte()
        #Affichage pression
        bouton_pression = tk.Button(self.root2, text = "Set pressure", width = 8, command = lambda:self.mode_pression())
        ...
        while self.bouton_click==False:
            ...
            self.canvas2.create_image(screen_width-screen_width/20,screen_height-screen_height/12,image=self.logo)
            self.root2.update()
